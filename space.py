...
# Load config file.
stream = open("preferencias.yaml", 'r')
...
str_in = """
         ...
         Considere contribuir para o BOT seguir melhorando... 
    ...
    >> Conf para melhor funcionamento...
    >> Monitor em 1920/1080
    >> Janela do jogo bem pequena (22,5cm / 17,5 cm)
    >> Acesse o Arquivo "settings.yaml" e configure suas preferencias!
...
def processLogin():
    dbg.console('Logando...', 'INFO', 'ambos')
    sys.stdout.flush()
    loginSPG()
    time.sleep(12)
    playSPG()
...
def loginSPG():
    ...
    if login_attempts > 3:
        dbg.console('Muitas tentativas de Login..., refreshing', 'ERROR', 'ambos')
        login_attempts = 0
        processLogin()
        return
    if clickBtn(images['connect-wallet'], name='connectWalletBtn', timeout = 10):
        dbg.console('Botao "Connect wallet" detectado, Logando!', 'INFO', 'ambos')
...
def playSPG():
    if clickBtn(images['play'], name='okPlay', timeout=8):
        dbg.console('PLAY encontrado','INFO', 'ambos')
...
def confirm():
    ...
    if len(positions(images['victory'], th['commom'])) > 0:
        if clickBtn(images['confirm-victory'], name='okVicBtn', timeout=2) or clickBtn(images['confirm-bf'], name='okVicBtn', timeout=2):
            dbg.console('VITORIA... Proximo BOSS!!','INFO', 'ambos')
            ...
            if st['key_waves'] == True: 
                if cont_boss == 9:
                    time.sleep(30) 
                    dbg.console("Boss 9, Reenviando Naves", 'DEBUG', 'ambos')
                    clickBtn(images['ship'])
                if cont_boss == 14:
                    time.sleep(30) 
                    dbg.console("Boss 14, Reenviando Naves", 'DEBUG', 'ambos')
                    clickBtn(images['ship'])
                if cont_boss == 19:
                    time.sleep(30) 
                    dbg.console("Boss 19, Reenviando Naves", 'DEBUG', 'ambos')
                    clickBtn(images['ship'])
    ...
    if st['send_ships_full'] == True:
        ...
        for (x, y, w, h) in not_working_green_bars:
            ...
            if ships_clicks >= st['qtd_send_spaceships']:
                dbg.console('Naves Prontas pra Peleia!', 'INFO', 'ambos')
                return -1
...
def screen_close():
    ...
    if clickBtn(images['close'],timeout=1):
        dbg.console('CLOSE encontrado!', 'ERROR', 'ambos')        
        cont_boss = 1
        confirm_click = True
    if clickBtn(images['bt-ok'], timeout=1):
        dbg.console('OK encontrado', 'ERROR', 'ambos')        
        cont_boss = 1
        confirm_click = True
...
def ships_15_15():
    ...
    while(not has_timed_out):
        ...
        if(len(matches)==0):
            ...
            continue
        dbg.console('Naves em 15-15, Refresh', 'DEBUG', 'ambos')
        return True
    return False
...
def refreshSpaceships(qtd):
    global cont_boss
    global ships_clicks
    dbg.console('Trocando esquadrão, novas naves pra Peleia!', 'INFO', 'ambos')
    buttonsClicked = 1
    go_to_boss = False
    cda =  100
    aux_ships = 0
    ...
    if ships_clicks > 0:
        dbg.console('Naves Prontas:' + str(ships_clicks), 'DEBUG', 'ambos')
        if ships_clicks == st['qtd_send_spaceships']:
...
def endFight():
    global cont_boss    
    cont_boss = 1
    dbg.console("FIM da batalha, mas não da guerra... ", 'INFO', 'ambos')
    time.sleep(3) 
    returnBase()
    time.sleep(15) 
...
def main():
    ...
    print(str_in)        
    time.sleep(5)
    dbg.console('BOT ON, Bons Sonhos :D  V: ' + str(VERSAO_SCRIPT), 'INFO', 'ambos')
    dbg.console('Total de naves: ' + str(st['empty_qtd_spaceships']), 'INFO', 'ambos')
    dbg.console('Quantidade de naves por turno: ' + str(st['qtd_send_spaceships']), 'INFO', 'ambos')
    ...
    while True:
        ...
        if action_found == False:
            dbg.console('Aguardando alguma ação...', 'WARNING', 'ambos')
